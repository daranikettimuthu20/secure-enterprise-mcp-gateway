...
A security-scanning reverse proxy for MCP (Model Context Protocol) tool calls.
Every tool invocation passes through authentication, RBAC, prompt-injection
detection, and PII/secrets scanning before it's forwarded to the real
upstream MCP server - and the response is scanned again on the way back.

## Why

MCP servers execute whatever arguments a model sends them, with no built-in
policy layer. This gateway adds one: it sits between the client/model and
your real tool servers and enforces who can call what, blocks prompt
injection payloads hiding in tool arguments, and stops PII/secrets from
...
Get a token and call a tool through the gateway:
